...
function drawCombo(type,vid){
    ...
    if(type == 1){
		createNewCombo();
		comboColumn = $('input:radio[name="comboColumn"]:checked').val();
		settings = comboColumn;
		$('#setting' + gadgetID).val(settings);
    }
    else if(type == 2){
		settings = $('#setting' + gadgetID).val();
        comboColumn = settings;
    }
    if(type == 3){
   	    comboColumn = $('input:radio[name="comboColumnEdit"]:checked').val();
        settings = comboColumn;
	    $('#setting' + gadgetID).val(settings);
    }
	...
	$.ajax({
		...
		type: 'POST',
		url: "getCombo.php",
		data: {'comboColumn':comboColumn,'titleNo':titleNo},
		...
		var JSONResponse = JSON_Response;
			...
			{
				data.addColumn('number',String(JSONResponse[i].Location));
			}
...
function generateCombo(a){
	var options={
		title: 'Combo Chart for ' + comboColumn,
		vAxis: {title : "Value"},
...
function createNewCombo(){
	...
	var gadget = "<div class='gadget' id='" + gadgetID + "' style='top: 50px; left:0px; width:400px; height: 300px' type='combo'>";
